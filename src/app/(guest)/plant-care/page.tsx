...
function SectionIcon({ type }: { type: string }) {
  const props = { className: "w-6 h-6 text-accent-green" };
  ...
  switch (type) {
    ...
    default:
      return <LeafIcon className="w-6 h-6 text-accent-green" />;
  }
}
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden min-h-[70vh] flex items-center">
        ...
        <div className="absolute right-[15%] top-[12%] w-24 h-24 rounded-full border border-accent-light/30 pointer-events-none" style={{ animation: "float 6s ease-in-out infinite" }} />
        <div className="absolute left-[10%] bottom-[18%] w-14 h-14 rounded-full bg-accent-light/10 pointer-events-none" style={{ animation: "float-slow 8s ease-in-out infinite" }} />
      ...
      <section className="relative border-t border-black/5 bg-accent-green/[2%] overflow-hidden">
        <div className="absolute -right-12 top-5 w-36 h-36 rounded-full border border-accent-light/10 pointer-events-none" />
        <div className="absolute left-[15%] bottom-0 w-24 h-24 rounded-full bg-accent-green/[2%] pointer-events-none" />
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          <h2 className="text-2xl md:text-3xl font-heading font-bold text-accent-light text-center mb-10">
            Plant Emergency?
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              <div
                key={tip.q}
                className="border border-accent-green/15 bg-bg-soft/30 rounded-xl p-4 text-center hover:bg-bg-soft/60 hover:border-accent-green/30 transition-all duration-300"
              >
                <LeafIcon className="w-6 h-6 text-accent-green/60 mx-auto mb-3" />
                <p className="text-sm font-medium text-accent-light mb-1">{tip.q}</p>
                <p className="text-xs text-accent-green/70">{tip.a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative border-t border-black/5 overflow-hidden">
        <div className="absolute -left-24 top-20 w-80 h-80 rounded-full bg-accent-green/[1%] pointer-events-none" />
        <div className="absolute right-0 bottom-10 w-48 h-48 rounded-full border border-accent-light/[3%] pointer-events-none" />
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="grid md:grid-cols-2 gap-6 md:gap-8">
            {guideSections.map((section, i) => (
              <div
                key={section.title}
                className="group rounded-2xl border border-accent-green/15 bg-bg-soft/30 p-8 hover:bg-bg-soft/60 hover:border-accent-green/30 hover:-translate-y-1 transition-all duration-300 animate-fade-in-up"
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <div className="flex items-center gap-4 mb-5">
                  <div className="w-12 h-12 rounded-xl bg-accent-light/15 flex items-center justify-center shrink-0 group-hover:bg-accent-light/25 group-hover:scale-110 transition-all">
                    <SectionIcon type={section.icon} />
                  </div>
                  <h2 className="text-xl font-heading font-semibold text-accent-green">
                    {section.title}
                  </h2>
                </div>
                <ul className="space-y-3">
                  {section.content.map((tip, j) => (
                    <li key={j} className="flex items-start gap-3 text-sm text-accent-green/70 leading-relaxed">
                      <span className="w-1.5 h-1.5 rounded-full bg-accent-light/60 mt-2 shrink-0" />
                      {tip}
                    </li>
                  ))}
                </ul>
